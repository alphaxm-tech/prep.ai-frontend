...
function OverviewTab({ student }: { student: Student }) {
  ...
  const { quizzes, interviews, coding, assessments, applications, resumes } =
    MOCK_STUDENT_DETAIL;
  ...
  const clearedAssessments = assessments.filter(
    (a) => a.result === "Cleared",
  ).length;
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <div className="space-y-0">
            {[
              {
                icon: "📝",
                text: `Completed "${quizzes[0].title}"`,
                sub: `Score: ${quizzes[0].score}%`,
                time: quizzes[0].date,
              },
              {
                icon: "🤖",
                text: `AI Interview — ${interviews[0].company}`,
                sub: `Rating: ${interviews[0].rating}/10`,
                time: interviews[0].date,
              },
              {
                icon: "💻",
                text: `Solved "${coding[0].problem}"`,
                sub: coding[0].language,
                time: coding[0].date,
              },
              {
                icon: "📄",
                text: `Updated Resume ${resumes[0].version}`,
                sub: resumes[0].template + " template",
                time: resumes[0].updatedAt,
              },
              {
                icon: "🏢",
                text: `Applied to ${applications[0].company}`,
                sub: applications[0].role,
                time: applications[0].appliedOn,
              },
            ].map((item, i, arr) => (
              <div key={i} className="flex items-start gap-3 group">
                <div className="flex flex-col items-center flex-shrink-0">
                  <div className="w-8 h-8 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center text-sm shadow-sm">
                    {item.icon}
                  </div>
                  {i < arr.length - 1 && (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="flex items-center gap-0">
          {(
            ...
            const count = MOCK_STUDENT_DETAIL.applications.filter(
              (a) => a.status === stage,
            ).length;
            ...
            return (
              <React.Fragment key={stage}>
                ...
                {i < 4 && (
                  <div
                    className={`flex-shrink-0 w-6 h-0.5 -mt-4 ${isDone ? "bg-yellow-400" : "bg-gray-200"}`}
                  />
                )}
              </React.Fragment>
            );
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {resumes.map((r) => (
          <div
            key={r.id}
            className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition-all duration-200 group"
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span
                    className={`inline-block mt-0.5 text-[11px] font-semibold px-2.5 py-0.5 rounded-lg border ${templateColors[r.template] ?? "bg-gray-100 text-gray-600 border-gray-200"}`}
                  >
                    {r.template}
                  </span>
                </div>
            ...
            <div className="flex gap-4 text-xs text-gray-400">
              <span>
                Created:{" "}
                <span className="font-medium text-gray-600">{r.createdAt}</span>
              </span>
              <span>
                Updated:{" "}
                <span className="font-medium text-gray-600">{r.updatedAt}</span>
              </span>
            </div>
          </div>
        ))}
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          {
            label: "Easy ✓",
            value: `${byDiff.Easy.filter((c) => c.status === "Solved").length}/${byDiff.Easy.length}`,
            color: "text-emerald-600",
            bg: "bg-emerald-50 border-emerald-100",
          },
          {
            label: "Medium ✓",
            value: `${byDiff.Medium.filter((c) => c.status === "Solved").length}/${byDiff.Medium.length}`,
            color: "text-yellow-700",
            bg: "bg-yellow-50 border-yellow-100",
          },
        ].map((s) => (
          ...
          <tbody>
            {coding.map((c) => (
              ...
              >
                ...
                <td className="py-3.5 px-5">
                  <span
                    className={`px-2.5 py-1 rounded-lg text-xs font-semibold border ${DIFFICULTY_BADGE[c.difficulty]}`}
                  >
                    {c.difficulty}
                  </span>
                </td>
                ...
                <td className="py-3.5 px-5">
                  <span
                    className={`flex items-center gap-1.5 text-xs font-semibold w-fit px-2.5 py-1 rounded-lg ${c.status === "Solved" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-yellow-50 text-yellow-700 border border-yellow-200"}`}
                  >
                    {c.status === "Solved" ? (
                      <CheckCircle2 className="w-3 h-3" />
                    ) : (
          ...
          <tbody>
            {applications.map((ap) => (
              ...
              >
                ...
                <td className="py-3.5 px-5">
                  <span
                    className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${APP_STATUS_BADGE[ap.status]}`}
                  >
                    {ap.status}
                  </span>
                </td>
          ...
          <div className="p-6 md:p-8">
            <div className="flex flex-col md:flex-row md:items-start gap-6">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-2">
                  <span className="px-3 py-1 bg-gray-100 text-gray-700 rounded-xl text-xs font-semibold">
                    {student.branch}
                  </span>
                  <span className="px-3 py-1 bg-blue-50 text-blue-700 border border-blue-100 rounded-xl text-xs font-semibold">
            ...
            <div className="mt-6 pt-5 border-t border-gray-100 grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                <div
                  key={kpi.label}
                  className={`flex items-center gap-3 px-4 py-3 rounded-2xl border ${kpi.bg}`}
                >
        ...
        <div className="flex items-center gap-1 bg-gray-50 border border-gray-100 rounded-2xl p-1 overflow-x-auto">
          {tabs.map((tab) => (
            ...
            >
              ...
              {tab.count !== undefined && (
                <span
                  className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${activeTab === tab.id ? "bg-yellow-100 text-yellow-700" : "bg-gray-200 text-gray-500"}`}
                >
                  {tab.count}
                </span>
              )}
