...
import { Course } from "@/server-api/api/types/super-admin.types";
import { useGetAllCourses } from "@/server-api/queries/ccg.queries";
...
export default function CoursesSelectionPage() {
  ...
  // create a new course locally (no create-course endpoint exists yet) and select it
  const handleCreateCourse = (input: NewCourseInput) => {
    const exists = allCourses.some(
      (c) => c.name.toLowerCase() === input.name.toLowerCase(),
    );
    if (exists) {
      setMessage({
        type: "error",
        text: `A course named "${input.name}" already exists.`,
      });
      return;
    }
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-50 via-white to-gray-50 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto">
          ...
          <main>
            <div className="bg-white ring-1 ring-gray-100 rounded-2xl shadow-xl p-6 sm:p-8">
              <div className="flex items-start justify-between gap-4 mb-2">
                ...
                <button
                  ...
                  className="shrink-0 px-4 py-2 rounded-full text-sm font-medium text-white bg-gradient-to-tr from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 transition"
                >
                  + Create new course
                </button>
              </div>
              ...
              <div
                className={`relative ${inputInvalid ? "mb-1" : "mb-4"}`}
                ref={comboRef}
              >
                ...
                <div className="flex items-center gap-3">
                  <div className="relative flex-1">
                    <input
                      ...
                      placeholder="Search or select a course (e.g., B.Tech Computer Science)"
                      className={`w-full px-4 py-3 pr-10 rounded-xl text-sm bg-gray-50 placeholder-gray-400 shadow-sm transition focus:outline-none focus:shadow-outline border ${
                        inputInvalid
                    ...
                    <button
                      ...
                      className="absolute right-2 top-1/2 -translate-y-1/2 w-6 h-6 grid place-items-center text-gray-400 hover:text-gray-600"
                    >
                      <svg
                    ...
                    {isDropdownOpen &&
                      ...
                      suggestions.length === 0 && (
                        <div className="absolute z-30 left-0 right-0 mt-2 bg-white border border-gray-100 rounded-xl shadow-lg px-4 py-2 text-xs text-gray-500">
                          No matching course. Press Add to create a new course
                          named "{query.trim()}"
                        </div>
                      )}
                  </div>
