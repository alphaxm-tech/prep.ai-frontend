...
export default function InterviewPage() {
  ...
  // countdown: number in seconds remaining; null means no countdown showing
  const [countdown, setCountdown] = useState<number | null>(3);

  // reset when company/title changes so interview restarts properly
  useEffect(() => {
    setCurrentIndex(0);
    setStarted(false);
    setPaused(false);
  ...
  // countdown effect: runs once on mount (or when countdown resets via company change)
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      if (seconds <= 0) {
        ...
        setTimeout(() => {
          ...
          setStarted(true); // start the interview automatically
        }, 250);
      }
    }, 1000);
  ...
  // when currentQuestion / started / paused / mute / rate changes, (re)speak if appropriate
  useEffect(() => {
